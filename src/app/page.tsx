...
export default function Dashboard() {
  ...
  async function fetchDashboardData() {
    try {
      ...
      setStats({
        orders: ordersCount || 0, workshops: workshopsCount || 0, pending: pendingCount || 0, shipping: shippingCount || 0, delivered: deliveredCount || 0, review: reviewCount || 0,
      });
  ...
  async function handleSaveWorkshopUpdate(id: string) {
    try {
      const { error } = await supabase.from('workshops').update({
        name: editWorkshopData.name, owner_name: editWorkshopData.owner_name, phone: editWorkshopData.phone, location: editWorkshopData.location_city
      }).eq('id', id);
      if (error) throw error;
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6 font-sans" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      {/* (بقية الترويسة والكروت كما هي في كودك الأصلي...) */}
      <header className="mb-8 border-b border-slate-800 pb-4 flex justify-between items-center flex-wrap gap-4">
        <div><h1 className="text-3xl font-bold text-white">{t.title}</h1></div>
        <button onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')} className="bg-slate-700 py-1.5 px-4 rounded-lg text-xs">{lang === 'ar' ? 'English' : 'عربي'}</button>
      </header>
      ...
      <div className="grid grid-cols-1 gap-8">
        <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-xl overflow-hidden">
          <div className="p-4 border-b border-slate-700 bg-slate-800/50 flex justify-between items-center">
            ...
            {/* -- الإضافة 2: حقل البحث -- */}
            <input type="text" placeholder="بحث بالاسم..." className="bg-slate-900 border border-slate-700 rounded p-1 text-sm text-white" onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-center">
              <thead className="bg-slate-900 text-slate-400 text-xs uppercase">
                <tr>
                  <th className="p-3">{t.name}</th><th className="p-3">{t.phone}</th><th className="p-3">الخدمة</th><th className="p-3">الملاحظات</th><th className="p-3">{t.city}</th><th className="p-3">{t.amount}</th><th className="p-3">{t.status}</th><th className="p-3">الإجراءات</th>
                </tr>
              </thead>
                    ...
                    <tr key={order.id} className="hover:bg-slate-700/30 transition-colors">
                      {isEditing ? (
                        <>
                          ...
                          <td className="p-2"><input type="number" value={editOrderData.total_amount} onChange={e => setEditOrderData({...editOrderData, total_amount: e.target.value})} className="bg-slate-900 text-white p-1 rounded text-xs w-16 border border-slate-600" /></td>
                          <td className="p-2"><select value={editOrderData.order_status} onChange={e => setEditOrderData({...editOrderData, order_status: e.target.value})} className="bg-slate-900 text-white p-1 rounded text-xs border border-slate-600"><option value="pending">Pending</option><option value="shipping">Shipping</option><option value="delivered">Delivered</option></select></td>
                          <td className="p-2"><button onClick={() => handleSaveOrderUpdate(order.id)} className="bg-emerald-600 px-2 py-1 rounded text-xs">حفظ</button></td>
                        </>
                      ) : (
                        <>
                          ...
                          <td className="p-3"><button onClick={() => startEditOrder(order)} className="text-sky-400 text-xs">تعديل</button></td>
                        </>
                      )}
                    </tr>
                  );
